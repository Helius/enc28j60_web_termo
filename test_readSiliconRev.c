...
uint16_t print_webpage(uint8_t *buf,uint8_t rev)
{
        ...
        plen=fill_tcp_data_p(buf,plen,PSTR("<center><p>Hello and Welcome ;)"));
        plen=fill_tcp_data_p(buf,plen,PSTR("<center><p>Helius's hardvare web-server AVR + ENC28J60 ver "));
        itoa((enc28j60getrev()),vstr,10);
        plen=fill_tcp_data(buf,plen,vstr);
        plen=fill_tcp_data_p(buf,plen,PSTR("</center><hr><br>gHelius@gmail.com\n"));
        return(plen);
}
